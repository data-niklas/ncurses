...
module NCurses
  class Window
    ...
    def resize(newrows, newcols)
      LibNCurses.wresize self, newrows, newcols
    end
    ...
    # Wrapper for `getcurx()`
    def x
      raise "getbegx error" if (out = LibNCurses.getbegx(self)) == ERR
      out
    end
    ...
    # Wrapper for `getcury()`
    def y
      raise "getbegy error" if (out = LibNCurses.getbegy(self)) == ERR
      out
    end
    ...
    # The cursor_y and cursor_x methods provide the cursor position of the current window
    def cursor_y
    ...
    def cursor_x
      raise "getcury error" if (out = LibNCurses.getcurx(self)) == ERR
      out
    end
    ...
    # Yields the Key | Char | Nil | MouseEvent, depending on the Event
    # Also the mouse_pressed method of EventWindows are fired, if they are being tracked and the mouse cursor is inside their boundaries 
    def get_char_delegate_mouse(&block)
      no_timeout
      loop do
        ch = get_char
        if ch == NCurses::Key::Mouse
            ...
            if !mouse.nil?
                ...
                y = coords[:y]
                z = coords[:z]
                EventWindow.windows.each do |window|
                    if x >= window.x && x < window.x + window.width && y >= window.y && y < window.y + window.height
                        window.mouse_pressed mouse.state, x-window.x, y-window.y, z, mouse.device_id
                    end
                end
                yield mouse
            else
    ...
    def clear_line(i : Int32 = nil)
        if !i.nil?
          move(i, 0)
        end
       LibNCurses.wclrtoeol self
    end
